...
@Aspect
@Component
@Order(1)  //这里order一定要小于tx:annotation-driven的order，即先执行DataSourceAspect切面，再执行事务切面，才能获取到最终的数据源
...
public class DataSourceAspect {
	 ...
	 */
	@Pointcut("execution(* com.zhuyong.test.aop.service..*.*(..))")
	public void aspect() {
		System.err.println("......DataSourceAspect.....aspect");
	}
	...
	public void before(JoinPoint joinPoint) {
		System.err.println("......DataSourceAspect.....before");
		...
		if (dataSource == null) {
			for (Class<?> clazz : target.getInterfaces()) {
				dataSource = this.getDataSource(clazz, method);
				if (dataSource != null) {  
	...
	@After("aspect()")
	public void after() {
		System.err.println("......DataSourceAspect.....after");
		//使用完记得清空
	...
	public DataSource getDataSource(Class<?> target, Method method) {
		try {
			...
			//1.优先方法注解
			Method m = target.getMethod(method.getName(), parameterTypes);
